 ...
 * @returns {Route[]}
 */
function mergeRoutes(target: any, source: any) {
  const routesMap: any = {};
  target.forEach((item: any) => (routesMap[item.path] = item));
  source.forEach((item: any) => (routesMap[item.path] = item));
 ...
 */
function formatAuthority(routes: any, pAuthorities: any[] = []) {
 ...
 * @param routesConfig 路由配置
 */
function loadRoutes({ router, store, i18n }, routesConfig: any) {
  ...
  if (asyncRoutes) {
    if (routesConfig && routesConfig.length > 0) {
      ...
      router.matcher = createRouterMatcher([], router.options);
 ...
 * @returns {Route[]}
 */
function deepMergeRoutes(target: any, source: any) {
  // 映射路由数组
  ...
  // 转换为 routes 数组
  const parseRoutesMap = (routesMap: any) => {
 ...
 * @returns {*}
 */
function getI18nKey(path: any) {
  const keys = path
    .split("/")
    .filter((item: any) => !item.startsWith(":") && item != "");
  keys.push("name");
  return keys.join(".");
}
 ...
 * @param options
 */
function loadGuards(guards: any, options: any) {
  const { beforeEach, afterEach } = guards;
  const { router } = options;
  beforeEach.forEach((guard: any) => {
  ...
  afterEach.forEach((guard: any) => {
    if (guard && typeof guard === "function") {
      router.afterEach((to: any, from: any) => guard(to, from, options));
    }
  });
}
