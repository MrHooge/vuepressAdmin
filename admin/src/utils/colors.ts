...
// 获取 ant design 色系
function getAntdColors(color: any, mode: any) {
	const options: any = mode && mode == themeMode.NIGHT ? { theme: 'dark' } : undefined;
	return generate(color, options);
}

// 获取功能性颜色
function getFunctionalColors(mode: any) {
	const options: any = mode && mode == themeMode.NIGHT ? { theme: 'dark' } : undefined;
...
// 获取菜单色系
function getMenuColors(color: any, mode: any) {
...
// 获取主题模式切换色系
function getThemeToggleColors(color: any, mode: any) {
	//主色系
...
export { isHex, isRgb, isRgba, toNum3, getAntdColors, getMenuColors, getThemeToggleColors, getFunctionalColors };
